...
// Branded email signature for Outlook drafts generated by the web app -
// each tenant sets their own (David, Aug 17: "let other clients also add
// their signature and branding").
router.route("/:id/updatesignature").post(async function (req, res) {
  try {
    var errResponse;
    const tenantId = req.params.id;
    const { signature } = req.body;

    var result = await TenantService.updateTenantSignature(tenantId, signature || {});
    if (result.reason) {
      return res.status(result.code).json(result);
    }
    if (result) {
